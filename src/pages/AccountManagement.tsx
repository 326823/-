      ...
      <div className="animate-slide-up" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <div className="table-toolbar" style={{ marginBottom: '24px' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)' }}>只有高级系统管理员有权进入此面板配置员工门禁与角色的鉴权账号</p>
          </div>
                    ...
                    users.map(u => (
                      <tr key={u.id}>
                        ...
                        <td>
                           ...
                           {u.username !== 'admin' && (
                             <button className="btn-icon danger" style={{ fontSize: '0.8rem' }} onClick={() => handleDelete(u.id, u.username)}>撤销吊销</button>
                           )}
                           {u.username === 'admin' && <span style={{fontSize:'0.75rem', color:'#aaa'}}>系统根基</span>}
                        </td>
                      </tr>
                  ))}
               </tbody>
